...
void MyWindow::s_keyboardCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if ((key == GLFW_KEY_SPACE || key == GLFW_KEY_P || key == GLFW_KEY_C || key == GLFW_KEY_N || GLFW_KEY_ESCAPE)
		&& action == GLFW_PRESS)
	{
...
void MyWindow::pollEvents()
{
	...
	if (glfwGetKey(m_pWindow, GLFW_KEY_LEFT) == GLFW_PRESS)       m_pMyApplication->handleMovement(MyApplication::KEY_LEFT);
	else if (glfwGetKey(m_pWindow, GLFW_KEY_RIGHT) == GLFW_PRESS) m_pMyApplication->handleMovement(MyApplication::KEY_RIGHT);
	else if (glfwGetKey(m_pWindow, GLFW_KEY_UP) == GLFW_PRESS)    m_pMyApplication->handleMovement(MyApplication::KEY_UP);
	else if (glfwGetKey(m_pWindow, GLFW_KEY_DOWN) == GLFW_PRESS)  m_pMyApplication->handleMovement(MyApplication::KEY_DOWN);
...
void MyWindow::keyboardEvent(int key)
{
	...
	{
		m_pMyApplication->switchInteractionMode();
	}
